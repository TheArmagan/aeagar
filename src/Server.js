...
class Server {
    ...
    ejectMass(client) {
        ...
        for (var i = 0; i < client.cells.length; i++) {
            ...
            var loss = this.mode.onEjectSizeLoss?.(cell, client) ?? this.config.ejectSizeLoss;
            ...
            if (this.config.ejectVirus) {
                ...
                ejected = new Entity.EjectedMass(this, null, pos, this.mode.onEjectSize?.(cell, client) ?? this.config.ejectSize);
            }
            ejected.color = cell.color;
            ejected.setBoost(this.mode.onEjectVelocity?.(cell, client) ?? this.config.ejectVelocity, angle);
